...
　　公司一般会搭建自己的Maven私有服务器，这时公司局域网中的机器会到私服上寻找jar包。使用私服的好处是：

　　（1）基本能保证无外网时也能正常构建项目；

　　（2）可将公司自己的jar包放在私服上，供局域网中所有机器使用。

　　有了私服，那么找jar包就会从本地找，如果找不到就到私服找，最后找中央仓库。搭建私服一般采用免费的Nexus软件，该软件由Sonatype提供。

　　这里就不介绍如何搭建私服以及如何配置了，需要时可自行查找资料。
