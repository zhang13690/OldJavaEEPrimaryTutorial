...
　　（1）生成的代码中有一个“_jspService”方法，该方法其实就对应普通Servlet的service方法。该方法中会将JSP中的普通文本内容用out.write()输出（out对象其实就是resp.getWriter()）

　　（2）JSP表达式中的内容会在_jspService方法中直接用out.print()进行输出。
