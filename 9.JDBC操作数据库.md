...
　　实际上，上面的做法其实可以说是Spring的“核心”。Spring是以后我们要重点学习的Java应用框架。Spring的核心是IOC，即控制反转，就是控制权的反转。例如上面的例子，本来Service层对DAO对象的依赖是通过我们自行创建对象解决了，而使用工厂后，对于Service来说，是工厂给它提供了一个对象，而不是自己创建的，这就“控制反转”了。学习完Spring后，会理解地更深刻。
